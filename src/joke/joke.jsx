...
let jokesTold = new Set();

export default function Joke({onClick}) {
  function getRandomIndex(jokes) {
  ...
  function getRandomJoke() {
    let index;
    do {
      index = getRandomIndex(jokes);
    } while (jokesTold.has(index));
    jokesTold.add(index);
    if (jokes.length === jokesTold.size) {
      jokesTold.clear();
    }
    return jokes[index];
  }
  ...
  return (
    <div className='joke' onClick={onClick}>
      {joke}
    </div>
  );
}
...
const jokes = [
  ...
  'Found out I was color blind the other day... That one came right out the purple.',
